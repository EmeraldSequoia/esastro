...
// These identifiers are incompatible with ESChronoConstants.hpp -- use only in .cpp files.
// but this is almost certainly backwards; this file is actually included from 
// chronometer/Classes/Constants.h which is included in nearly every ObjC file there.

// There was a ported version of this file in ESChronoConstants.h but that seems like a
// big mistake in retrospect, as there is no reason for it and it forced the data here
// to be defined in two places.  Moving back to a single version with EC-style names now.

// This also contains a merge of ESAstroConstants.hpp, a file created in May 2011 when creating
// the portable esastro library used in the factored version of EO.

#ifndef ESASTRO_EC_CONSTANTS_H
#define ESASTRO_EC_CONSTANTS_H
...
#define EC_SCALE_GL
#undef EC_STORE
#define EC_MAX_WATCHES 40  // controls size of stack array in SortWatchDescriptorArray
...
#define ECMinSwipeXIPad			130	// more than this counts 
...
// ESAstroConstants had 0.01 for kECTimerResolutionInSeconds below
#define kECTimerResolutionInSeconds	(0.005)	// all events are rounded to this number of seconds
...
#define ECZoomMaxLogicalPower2 ( 0)  // 1.0
#ifdef EC_ARCHIVEHD
#define ECZoomMaxPower2 ( 2)  // 4.0
#else
#define ECZoomMaxPower2 ( 1)  // 2.0
#endif

// Derived from above
#define ECNumVisualZoomFactors (ECZoomMaxPower2 - ECZoomMinPower2 + 1)
#define ECNumLogicalVisualZoomFactors (ECZoomMaxLogicalPower2 - ECZoomMinPower2 + 1)
...
#define kECButtonFasterRepeatInterval	0.02
...
#define ECAlarmTriangle			@"Triangle"  // .wav
#define ECAlarmTriangleRings		20
#define ECAlarmTriangleRepeat		0.5
#define ECAlarmChime			@"Chime"
#define ECAlarmChimeRings		5
#define ECAlarmChimeRepeat		2

#define ECInitialAccuracy		10000	// locationManager desired accuracy on startup (meters)
#define ECRequestAccuracy		10000	// locationManager desired accuracy on user requests (meters)
...
#define ECLocationRequestCheckTimeout   60      // seconds after which we stop trying to get a location and just display yellow
...
typedef enum ECCalendarWheelType {
    ECNotCalendarWheel              = -1,
    ECCalendarWheel012B             = 0,
    ECCalendarWheel3456             = 1,
    ECCalendarWheelOct1582          = 2
} ECCalendarWheelType;

#define ECCalendarWheelSpokeExtension 20

#define ECUseISO8601WeekNumber true

typedef enum ECCalendarRowCoverType {
    ECCalendarCoverRow56Right       = 0,
    ECCalendarCoverRow6Left         = 1,
    ECCalendarCoverRow1Right        = 2,
    ECCalendarCoverRow1Left         = 3
} ECCalendarRowCoverType;
#define ECCalendarRowCoverMin (ECCalendarCoverRow56Right)
#define ECCalendarRowCoverMax (ECCalendarCoverRow1Left)
...
typedef enum ECButtonEnabledControl {
    ...
    ECButtonEnabledBogusValueToShutUpStupidCompiler = 9999
...
#define kECmaxUpdate			(60*60*24*365*10)		    // 10 years  (was 1 hour in EC)
...
// For Android "Status Bar" positioning
typedef enum ECAndroidStatusBarLocation {
    ECAndroidStatusBarUnknown,
    ECAndroidStatusBarTop,
    ECAndroidStatusBarCenter,
    ECAndroidStatusBarBottom
} ECAndroidStatusBarLocation;

#endif  // ESASTRO_EC_CONSTANTS_H
