...
//
//  Copied from Emerald Chronometer 22 May 2011
//
...
#include "Lunar/ESWBLunarTable.h"
#include "Planets/ESWBPlanetsTable.h"

#include "../src/ESAstroConstants.hpp"
//#include "../src/ESAstronomy.hpp"
#include "../src/ESAstronomyCache.hpp"
...
// COPY OF CODE FROM ECAstronomy.m for testing purposes...
...
r// return in centuries since 2000 epoch
...
double WB_sunLongitudeRaw(double       hundredCenturiesSinceEpochTDT,
	...
	longitude = ESUtil::fmod(longitude, M_PI * 2);
	if (currentCache) {
...
void WB_sunRAAndDecl(double 	  hundredCenturiesSinceEpochTDT,
    ...
    apparentLongitude = ESUtil::fmod(apparentLongitude, M_PI * 2);
...
static void WB_convertGeocentric(double hundredCenturiesSinceEpochTDT,
	...
	*apparentRightAscension = ESUtil::fmod(atan2(y, x), M_PI * 2);
...
double WB_planetHeliocentricLongitude(int    	   planetNumber,
      ...
      case ECPlanetEarth:
	{
	    ...
	    double helioLong = ESUtil::fmod(M_PI + sunMeanLongitude, 2 * M_PI);
	    if (helioLong < 0) {
		helioLong += 2 * M_PI;
	    }
	    return helioLong;
...
void someFunctionToUseAllThosePlanetDescriptorsToShutUpTheCompiler() {
    printf("0x%08lx\n", (unsigned long)&mercuryDescriptor);
    printf("0x%08lx\n", (unsigned long)&venusDescriptor);
    printf("0x%08lx\n", (unsigned long)&marsDescriptor);
    printf("0x%08lx\n", (unsigned long)&jupiterDescriptor);
    printf("0x%08lx\n", (unsigned long)&saturnDescriptor);
    printf("0x%08lx\n", (unsigned long)&uranusDescriptor);
    printf("0x%08lx\n", (unsigned long)&neptuneDescriptor);
}

#ifndef NDEBUG

#ifndef STANDALONE
static std::string formattedSize(size_t size) {
    if (size % (1024 * 1024) == 0) {
	return ESUtil::stringWithFormat("%d  MB  ", size / (1024 * 1024));
    }
    return ESUtil::stringWithFormat("%uld", size);
}

static void printMemoryLine(size_t            size,
                            const std::string &description) {
    std::string formattedString = formattedSize(size);
    printf("%12s : %s\n", formattedString.c_str(), description.c_str());
}
...
static void printInnerPlanet(const InnerPlanetDescriptor *descriptor,
    ...
    printMemoryLine(longitudeSize, ESUtil::stringWithFormat("%7s long: %3d %2d-byte datum elements", planetName, descriptor->numLongData, sizeof(InnerPlanetDatum)));
    *totalMemory += longitudeSize;
    size_t latitudeSize = descriptor->numLatData * sizeof(InnerPlanetDatum);
    printMemoryLine(latitudeSize, ESUtil::stringWithFormat("%7s lat:  %3d %2d-byte datum elements", planetName, descriptor->numLatData, sizeof(InnerPlanetDatum)));
    *totalMemory += latitudeSize;
    size_t radiusSize = descriptor->numRadData * sizeof(InnerPlanetDatum);
    printMemoryLine(latitudeSize, ESUtil::stringWithFormat("%7s rad:  %3d %2d-byte datum elements", planetName, descriptor->numRadData, sizeof(InnerPlanetDatum)));
...
static void printOuterPlanet(const OuterPlanetDescriptor *descriptor,
    ...
    printMemoryLine(sz, ESUtil::stringWithFormat("%7s    :  %3d %2d-byte half-decade entries", planetName, descriptor->numEntries, sizeof(OuterPlanetDatum)));
    *totalMemory += sz;		    

    sz = descriptor->numEntries * sizeof(OuterPlanetJDRange);
    printMemoryLine(sz, ESUtil::stringWithFormat("%7s    :  %3d %2d-byte half-decade jd range elements", planetName, descriptor->numEntries, sizeof(OuterPlanetJDRange)));
    *totalMemory += sz;		    
}
...
void WB_printMemoryUsage() {
    ...
    printMemoryLine(sunDataSize, ESUtil::stringWithFormat("    Sun:  %3d %2d-byte datum elements", numSunData, sunDatumSize));
    totalMemory += sunDataSize;

    size_t thisSize = 218 * sizeof(SvDatum);
    printMemoryLine(thisSize,    ESUtil::stringWithFormat("Moon Sv:  %3d %2d-byte datum elements", 218, sizeof(SvDatum)));
    totalMemory += thisSize;

    thisSize = 244 * sizeof(Sv1Datum);
    printMemoryLine(thisSize,    ESUtil::stringWithFormat("Moon Sv1: %3d %2d-byte datum elements", 244, sizeof(Sv1Datum)));
    totalMemory += thisSize;

    thisSize = 154 * sizeof(Sv2Datum);
    printMemoryLine(thisSize,    ESUtil::stringWithFormat("Moon Sv2: %3d %2d-byte datum elements", 154, sizeof(Sv2Datum)));
    totalMemory += thisSize;

    thisSize = 25 * sizeof(Sv3Datum);
    printMemoryLine(thisSize,    ESUtil::stringWithFormat("Moon Sv3: %3d %2d-byte datum elements",  25, sizeof(Sv3Datum)));
    totalMemory += thisSize;

    thisSize = 188 * sizeof(SuDatum);
    printMemoryLine(thisSize,    ESUtil::stringWithFormat("Moon Su:  %3d %2d-byte datum elements", 188, sizeof(SuDatum)));
    totalMemory += thisSize;

    thisSize =  64 * sizeof(Su1Datum);
    printMemoryLine(thisSize,    ESUtil::stringWithFormat("Moon Su1: %3d %2d-byte datum elements", 64, sizeof(Su1Datum)));
    totalMemory += thisSize;

    thisSize =  64 * sizeof(Su2Datum);
    printMemoryLine(thisSize,    ESUtil::stringWithFormat("Moon Su2: %3d %2d-byte datum elements", 64, sizeof(Su2Datum)));
    totalMemory += thisSize;

    thisSize = 12 * sizeof(Su3Datum);
    printMemoryLine(thisSize,    ESUtil::stringWithFormat("Moon Su3: %3d %2d-byte datum elements", 12, sizeof(Su3Datum)));
    totalMemory += thisSize;

    thisSize = 154 * sizeof(SrDatum);
    printMemoryLine(thisSize,    ESUtil::stringWithFormat("Moon Sr:  %3d %2d-byte datum elements", 154, sizeof(SrDatum)));
    totalMemory += thisSize;

    thisSize = 114 * sizeof(Sr1Datum);
    printMemoryLine(thisSize,    ESUtil::stringWithFormat("Moon Sr1: %3d %2d-byte datum elements", 114, sizeof(Sr1Datum)));
    totalMemory += thisSize;

    thisSize =  68 * sizeof(Sr2Datum);
    printMemoryLine(thisSize,    ESUtil::stringWithFormat("Moon Sr2: %3d %2d-byte datum elements", 68, sizeof(Sr2Datum)));
    totalMemory += thisSize;

    thisSize = 19 * sizeof(Sr3Datum);
    printMemoryLine(thisSize,    ESUtil::stringWithFormat("Moon Sr3: %3d %2d-byte datum elements", 19, sizeof(Sr3Datum)));
    totalMemory += thisSize;
